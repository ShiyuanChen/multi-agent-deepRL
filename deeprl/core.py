...
class ReplayMemory:
    ...
    def __init__(self, max_size, window_length):
        ...
        self.replay_memory = RingBuffer(max_size)

    def append(self, state, action, reward, next_state, is_terminal):
        self.replay_memory.append(Sample(state, action, reward, next_state, is_terminal))
    ...
    def sample(self, batch_size, indexes=None):
        if indexes is None:
            r = xrange(0, len(self.replay_memory))
            indexes = random.sample(r, batch_size)

        samples = []
        for idx in indexes:
            samples.append(self.replay_memory[idx])
        return samples


    def clear(self):
        self.replay_memory = RingBuffer(self.max_size)
...
class RingBuffer(object):
    """RingBuffer for the memory
    Methods
    -------
    append(entry)
      Add a sample to the ring buffer. The sample can be any python
      object.
    """
    def __init__(self, max_size):
        ...
        self.length = 0
        self.max_size = max_size
        self.entry = [None] * max_size

    def __len__(self):
        return self.length

    def __getitem__(self, index):
        return self.entry[(self.first_entry + index) % self.max_size]
    ...
    def append(self, entry):
        if self.length < self.max_size:
            ...
            self.first_entry += 1
            if self.first_entry >= self.max_size:
                self.first_entry = 0

        self.entry[(self.first_entry + self.length - 1) % self.max_size] = entry
